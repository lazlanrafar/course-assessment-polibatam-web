<template>
  <layout-app>
    <ContentHeader header="Setup" title="Support Level to SO" />
...
export default {
  ...
  computed: {
    ...
    reports() {
      return this.$store.state.studentOutcomeLevel.reports;
    },
    optionsTable: {
      get() {
        return this.$store.state.studentOutcomeLevel.optionsTable;
      },
      set(value) {
        this.$store.commit("SET_OPTIONS_TABLE_STUDENT_OUTCOME_LEVEL", value);
      },
    },
  },
  methods: {
    ...
    handleUpdate(id) {
      this.$store.dispatch("SetFormUpdateStudentOutcomeLevel", id);
      this.$store.commit("SET_IS_UPDATE_STUDENT_OUTCOME_LEVEL", id);
      this.handleModalForm(true);
    },
  },
  mounted() {
    this.$store.dispatch("GetStudentOutcomeLevel");
  },
};
</script>
