<template>
  <v-card :loading="isLoading" class="card shadow-none border">
    <div class="card-header fw-medium">Percentage of Student within each Proficiency Level</div>
    <div class="card-body">
      <div class="table-responsive">
        <table class="table table-bordered fs-12" v-if="report.course">
          <thead>
            <tr class="table-active fw-medium">
              <td rowspan="3">Proficiency Level</td>
          ...
          <tbody>
            <tr v-for="(item, i) in percentage_of_student_within_each_proficiency_level" :key="i">
              <td>
                {{ item.level }}.
                {{ item.description }}
              </td>
...
<script>
export default {
  name: "PercentageOfStudentWithinEachProficiencyLevel",
  computed: {
    report() {
      return this.$store.state.assessment.report;
    },
    isLoading() {
      return this.$store.state.assessment.loading.percentage_of_student_within_each_proficiency_level;
    },
    percentage_of_student_within_each_proficiency_level() {
      return this.$store.state.assessment.percentage_of_student_within_each_proficiency_level;
    },
  },
  mounted() {
    this.$store.dispatch("GetPercentageOfStudentWithinEachProficiencyLevel", this.$route.params.id);
  },
};
</script>
